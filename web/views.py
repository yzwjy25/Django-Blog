...
def article(request):
    ...
    blog = Blog.objects.filter(**query_con).annotate(c=Count("comment")).values("id", "title", "content_sample",
                                                                                "category__title", "read_count",
                                                                                "publish_time", "c", 'img')
...
def detial(request, blog_id):
    ...
    conn.hincrby('read_count',blog.id,  1)

    if request.method == "POST":
        if not request.user.username:
            return render(request, "web/info.html", {"message": "您未登录，请先登录。"})
        post_dict = request.POST.dict()
        Comment.objects.create(user_id=request.user.id, content=post_dict.get('editorContent'),
                               blog_id=post_dict.get('articleid'), publish_time=datetime.datetime.now())
...
def register(request):
    ...
    if form.is_valid():
        send_email_celery.delay('欢迎您注册', '请点击下面链接激活您的账户:{}/{}/'.format(settings.SERVER_NAME + "/active", token),
